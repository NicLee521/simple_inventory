...
class _ImportExportMixin(_CoordinatorProtocol):
    ...
    def _build_import_payload(self, item_data: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            FIELD_SERVINGS_PER_UNIT: float(item_data.get(FIELD_SERVINGS_PER_UNIT, 0) or 0),
            FIELD_CALORIES_PER_SERVING: float(item_data.get(FIELD_CALORIES_PER_SERVING, 0) or 0),
            FIELD_PROTEIN_G_PER_SERVING: float(item_data.get(FIELD_PROTEIN_G_PER_SERVING, 0) or 0),
            FIELD_CARBS_G_PER_SERVING: float(item_data.get(FIELD_CARBS_G_PER_SERVING, 0) or 0),
            FIELD_FAT_G_PER_SERVING: float(item_data.get(FIELD_FAT_G_PER_SERVING, 0) or 0),
            FIELD_SERVING_SIZE: str(item_data.get(FIELD_SERVING_SIZE, "")),
    ...
    def _items_to_csv(self, items: list[dict[str, Any]]) -> str:
        ...
        for item in items:
            row = {
                ...
                "servings_per_unit": item.get(FIELD_SERVINGS_PER_UNIT, 0),
                "calories_per_serving": item.get(FIELD_CALORIES_PER_SERVING, 0),
                "protein_g_per_serving": item.get(FIELD_PROTEIN_G_PER_SERVING, 0),
                "carbs_g_per_serving": item.get(FIELD_CARBS_G_PER_SERVING, 0),
                "fat_g_per_serving": item.get(FIELD_FAT_G_PER_SERVING, 0),
                "serving_size": item.get(FIELD_SERVING_SIZE, ""),
    ...
    def _csv_to_items(self, csv_string: str) -> list[dict[str, Any]]:
        ...
        for row in reader:
            item: dict[str, Any] = {
                ...
                FIELD_SERVINGS_PER_UNIT: float(row.get("servings_per_unit", 0) or 0),
                FIELD_CALORIES_PER_SERVING: float(row.get("calories_per_serving", 0) or 0),
                FIELD_PROTEIN_G_PER_SERVING: float(row.get("protein_g_per_serving", 0) or 0),
                FIELD_CARBS_G_PER_SERVING: float(row.get("carbs_g_per_serving", 0) or 0),
                FIELD_FAT_G_PER_SERVING: float(row.get("fat_g_per_serving", 0) or 0),
                FIELD_SERVING_SIZE: row.get("serving_size", ""),
